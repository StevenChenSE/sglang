"""v3: two-pass verify_prefix_stage1 for HEAD_DIM=256 on gfx1100.

Pass A (max): per split, run the QK dots and record only e_max (no exp, no
PV accumulation, no acc registers) -> tiny register footprint, short chain.
Pass B (acc): re-run QK with e_max FIXED from pass A -> p = exp(qk - max)
has no serial rescale dependency; iterations pipeline and acc += is
associative. Combine stage2 unchanged (needs max(lse) semantics).
Offline: baseline v2 = 445.5us at BN32/W4/S8/ST2 (JOURNAL 12.55).
"""
import torch
import triton
...
def _verify_prefix_maxpass(
    ...
    tl.store(
        Att_Max + cur_batch * stride_mb + cur_head * stride_mh
        + split_kv_id * stride_ms + offs_l,
        e_max, mask=mask_l,
    )


@triton.jit
def _verify_prefix_accpass(
    Q, K_Buffer, V_Buffer,
    sm_scale, k_scale, v_scale,
    qo_indptr, kv_indptr, kv_indices,
    Att_Max, Att_Out, Att_Lse,
    stride_qbs, stride_qh,
    stride_buf_kbs, stride_buf_kh,
