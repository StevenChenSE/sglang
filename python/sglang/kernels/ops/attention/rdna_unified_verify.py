...
# Vendored from vLLM's triton_unified_attention_diffkv (see JOURNAL.md
# 12.134-12.135): adopted for the gfx1100 spec-verify depth-decay fix.
# Surgical changes vs upstream:
#   FIX-2 (12.135 defect 2): block->sequence resolution uses a
#     host-precomputed per-sequence q-block offset table
#     (``q_block_offsets_ptr``) instead of ``cu[i]//BLOCK_Q + i``,
#     which is off-by-one whenever a preceding sequence's query length
#     divides BLOCK_Q exactly (our MLE=4 / BLOCK_Q=2 verify shape).
#   The caller must pad metadata buffers (their backend pads to
#     seq_threshold_3D rows) — defect 1 avoidance lives in the adapter.
...
from sglang.kernels.ops.attention._rdna_verify_helpers import (
...
def _resolve_by_offsets(
    ...
    """FIX-2: map a global q-block index to its sequence via the
    per-sequence block-offset prefix table. Returns the same tuple as
    upstream ``resolve_seq_and_query_len``."""
    seq_idx = 0
    for s in range(num_seqs):
        start = tl.load(q_block_offsets_ptr + s)
        end = tl.load(q_block_offsets_ptr + s + 1)
        seq_idx = tl.where(
            (q_block_global_idx >= start) & (q_block_global_idx < end), s, seq_idx
        )
    q_block_start_idx = tl.load(q_block_offsets_ptr + seq_idx)
    q_block_local_idx = q_block_global_idx - q_block_start_idx
    cur_batch_in_all_start_index = tl.load(query_start_len_ptr + seq_idx)
    cur_batch_query_len = (
        tl.load(query_start_len_ptr + seq_idx + 1) - cur_batch_in_all_start_index
    )
...
def kernel_reduce_segments_diffkv(
    ...
    # Floor: all-masked padding lanes load -inf and exp(-inf - -inf)=NaN would poison expsum/output.
...
def unified_attention_diffkv(
    ...
    # FIX-2: per-sequence q-block offset prefix (int32 [num_seqs+1]) and
    # the exact total block count (static under CUDA-graph capture).
    q_block_offsets: torch.Tensor | None = None,
    total_q_blocks: int | None = None,
    ...
    # FIX-2: block offsets per sequence (prefix of ceil(q_len_b / BLOCK_Q)).
    # Callers with a fixed query length per seq (spec-verify: constant MLE)
    # pass ``q_block_offsets`` precomputed once (graph-safe, no D2H sync).
    if q_block_offsets is None:
        q_lens = cu_seqlens_q[1:] - cu_seqlens_q[:-1]
        q_block_offsets = torch.zeros(
            num_seqs + 1, dtype=torch.int32, device=q.device
        )
        q_block_offsets[1:] = (
            torch.div(q_lens + BLOCK_Q - 1, BLOCK_Q, rounding_mode="floor")
            .to(torch.int32)
            .cumsum(0)
        )
    if total_q_blocks is None:
        # Host-side fast path: constant per-seq query length => exact.
        # (Validation callers only; the adapter always passes it.)
        q_lens = (cu_seqlens_q[1:] - cu_seqlens_q[:-1]).tolist()
        total_q_blocks = sum((l + BLOCK_Q - 1) // BLOCK_Q for l in q_lens)
    total_num_q_blocks = total_q_blocks
    ...
    kernel_unified_attention_diffkv[grid](
        ...
        q_block_offsets_ptr=q_block_offsets,
        BLOCK_Q=BLOCK_Q,
        num_seqs=num_seqs,
        BLOCK_M=BLOCK_M,
