...
__global__ void moe_gemm_q4_kernel_rdna3(
  ...
  // moe_align_block_size buckets invalid experts in bucket `num_experts`
  // (and some producers still emit -1); either would index weights past
  // b_q_weight. The entry derives the bound from b_q_weight.size(0)
  // (REVIEW 2026-09-10 H5).
  ...
  #pragma unroll
    for (int j = 0; j < 4; ++j) {
      ...
      if constexpr (std::is_same<T, half>::value) {
        ...
        half2 dq[4][4];
        gptq_rdna3::dequant_4bit_8_fp16((uint32_t)b_w[j].x, dq[0], z_h[0],
                                        y_h[0]);
        gptq_rdna3::dequant_4bit_8_fp16((uint32_t)b_w[j].y, dq[1], z_h[1],
                                        y_h[1]);
        gptq_rdna3::dequant_4bit_8_fp16((uint32_t)b_w[j].z, dq[2], z_h[2],
                                        y_h[2]);
        gptq_rdna3::dequant_4bit_8_fp16((uint32_t)b_w[j].w, dq[3], z_h[3],
                                        y_h[3]);
...
void launch_moe_gemm_q4(
    ...
    int output_topk, int num_experts, cudaStream_t stream) {
  ...
  moe_gemm_q4_kernel_rdna3<T, BLOCK_SIZE_M><<<grid, block, 0, stream>>>(
      ...
      mul_topk_weight, output_topk, num_experts);
}
...
void dispatch_moe_gemm_q4(
    ...
    bool mul_topk_weight, int output_topk, int num_experts,
    cudaStream_t stream) {
  ...
  switch (block_size_m) {
    case 1:
      launch_moe_gemm_q4<T, 1>(
          ...
          expert_zeros_stride, mul_topk_weight, output_topk, num_experts,
          stream);
      break;
    case 2:
...
void moe_gptq_gemm_rdna3(torch::Tensor a, torch::Tensor c,
                         ...
                         int64_t output_topk) {
#ifndef SGL_IS_RDNA
  TORCH_CHECK(false, "moe_gptq_gemm_rdna3 is only supported on RDNA architectures");
#endif
  ...
  auto dispatch = [&](auto* a_ptr, auto* c_ptr, const auto* s_ptr) {
    ...
    vllm::moe_gptq_rdna3::dispatch_moe_gemm_q4<T>(
        ...
        expert_weight_stride, expert_scales_stride, expert_zeros_stride,
        mul_topk_weight, (int)output_topk, (int)b_q_weight.size(0), stream);
  };
