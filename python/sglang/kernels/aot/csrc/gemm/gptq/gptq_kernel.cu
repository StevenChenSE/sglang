...
void shuffle_exllama_weight(uint32_t* q_weight, int* q_perm, int height, int width, int bit) {
  if (q_perm) {
    ...
    void (*kernel)(const uint32_t*, uint32_t*, const int*, const int) = make_sequential_4bit_kernel;
    if (bit == 2) {
  ...
  gridDim.x = DIVIDE(width, THREADS_X);
  gridDim.y = 1;
  void (*shuffle_kernel)(uint32_t*, const int, const int) = shuffle_4bit_kernel;
  if (bit == 2) {
...
torch::Tensor gptq_gemm(
    ...
    bool use_shuffle,
    int64_t bit, bool use_v2_format) {
#ifdef SGL_IS_RDNA
  TORCH_CHECK(use_shuffle, "RDNA3 GPTQ kernel requires use_shuffle=true (standard GPTQ format)");
  TORCH_CHECK(bit == 4, "Only 4-bit GPTQ is supported on RDNA3");
  return gptq_gemm_rdna3(a, b_q_weight, b_gptq_qzeros, b_gptq_scales, b_g_idx, use_v2_format);
#else
