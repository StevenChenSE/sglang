 ...
 public:
  ...
  static int maximum_active_blocks(int smem_capacity = -1) {
    ...
    if (smem_size <= (48 << 10)) {
      ...
      if (result != hipSuccess) {
        CUTLASS_TRACE_HOST(
            "  hipOccupancyMaxActiveBlocksPerMultiprocessor() returned error " << hipGetErrorString(result));
  ...
  Status initialize(Arguments const& args, void* workspace = nullptr, hipStream_t stream = nullptr) {
    ...
    if (smem_size >= (48 << 10)) {
      hipError_t result =
          hipFuncSetAttribute(Kernel<GemmKernel>, hipFuncAttributeMaxDynamicSharedMemorySize, smem_size);
