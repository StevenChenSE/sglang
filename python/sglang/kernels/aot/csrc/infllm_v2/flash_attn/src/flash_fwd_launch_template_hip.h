          ...
          {  // TODO remove debug info
            ...
            constexpr static bool Is_softcap = false;
            {
              ...
              if (smem_size >= 48 * 1024) {
                C10_CUDA_CHECK(hipFuncSetAttribute(kernel, hipFuncAttributeMaxDynamicSharedMemorySize, smem_size));
              }
              // int ctas_per_sm;
  ...
  if (params.num_splits > 1) {
    ...
    EVENK_SWITCH(is_even_K, IsEvenKConst, [&] {
      if (params.num_splits <= 2) {
       hipLaunchKernelGGL(( flash_fwd_splitkv_combine_kernel<Kernel_traits, kBlockM, 1, IsEvenKConst>)
            , dim3(grid_combine), dim3(Kernel_traits::kNThreads), 0, stream, params);
      } else if (params.num_splits <= 4) {
       hipLaunchKernelGGL(( flash_fwd_splitkv_combine_kernel<Kernel_traits, kBlockM, 2, IsEvenKConst>)
            , dim3(grid_combine), dim3(Kernel_traits::kNThreads), 0, stream, params);
      } else if (params.num_splits <= 8) {
       hipLaunchKernelGGL(( flash_fwd_splitkv_combine_kernel<Kernel_traits, kBlockM, 3, IsEvenKConst>)
            , dim3(grid_combine), dim3(Kernel_traits::kNThreads), 0, stream, params);
      } else if (params.num_splits <= 16) {
       hipLaunchKernelGGL(( flash_fwd_splitkv_combine_kernel<Kernel_traits, kBlockM, 4, IsEvenKConst>)
            , dim3(grid_combine), dim3(Kernel_traits::kNThreads), 0, stream, params);
      } else if (params.num_splits <= 32) {
       hipLaunchKernelGGL(( flash_fwd_splitkv_combine_kernel<Kernel_traits, kBlockM, 5, IsEvenKConst>)
            , dim3(grid_combine), dim3(Kernel_traits::kNThreads), 0, stream, params);
      } else if (params.num_splits <= 64) {
       hipLaunchKernelGGL(( flash_fwd_splitkv_combine_kernel<Kernel_traits, kBlockM, 6, IsEvenKConst>)
            , dim3(grid_combine), dim3(Kernel_traits::kNThreads), 0, stream, params);
      } else if (params.num_splits <= 128) {
       hipLaunchKernelGGL(( flash_fwd_splitkv_combine_kernel<Kernel_traits, kBlockM, 7, IsEvenKConst>)
            , dim3(grid_combine), dim3(Kernel_traits::kNThreads), 0, stream, params);
      }
      C10_CUDA_KERNEL_LAUNCH_CHECK();
    });
...
template <typename Kernel_traits, bool Is_causal>
void run_flash_splitkv_fwd_stage1(Flash_fwd_params& params, hipStream_t stream) {
  static_assert(!Kernel_traits::Is_Q_in_regs, "SplitKV implementation does not support Is_Q_in_regs");
  static_assert(!Kernel_traits::Share_Q_K_smem, "SplitKV implementation does not support Share_Q_K_smem");
  constexpr size_t smem_size = Kernel_traits::kSmemSize;
  const int num_m_block = (params.seqlen_q + Kernel_traits::kBlockM - 1) / Kernel_traits::kBlockM;
