...
class CustomAllreduce {
 public:
  ...
  char* open_ipc_handle(const void* ipc_handle) {
    ...
    if (new_handle) {
      char* ipc_ptr;
      CHECK_CUDA_SUCCESS(hipIpcOpenMemHandle(
  ...
  register_graph_buffers(const std::vector<std::string>& handles, const std::vector<std::vector<int64_t>>& offsets) {
    ...
    CHECK_CUDA_SUCCESS(
        hipMemcpy(d_rank_data_base_, rank_data.data(), sizeof(RankData) * num_buffers, hipMemcpyHostToDevice));
...
#define KL(ngpus, name)hipLaunchKernelGGL(( name<T, ngpus>), dim3(blocks), dim3(threads), 0, stream, ptrs, sg_, self_sg_, output, rank_, size);
...
#define REDUCE_CASE(ngpus)                                                                                         \
  case ngpus: {                                                                                                    \
    if constexpr (!std::is_same<T, float>::value) {                                                                \
     hipLaunchKernelGGL(( custom_all_reduce_2shot<T, ngpus>), dim3(blocks), dim3(threads), 0, stream, ptrs, sg_, self_sg_, output, rank_, size); \
    } else {                                                                                                       \
      if ((world_size_ <= kAllReduceGPUSmall && bytes < kAllReduceSmallThreshold) ||                               \
          (world_size_ <= kAllReduceGPULarge && bytes < kAllReduceLargeThreshold)) {                               \
        KL(ngpus, cross_device_reduce_1stage);                                                                     \
