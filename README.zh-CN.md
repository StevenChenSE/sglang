...
| **速度留存率 (16k / 0k)** | **83.6%** | 85.7% | 87.5% | 两次运行取平均 |
...
|---|:---:|:---:|:---:|:---:|
| **平均生成速度 (Mean TG)** | **89.46 tok/s** | 63.50 tok/s | 67.96 tok/s | **提速 +40.9%** |
| **中位数速度 (Median TG)** | **88.38 tok/s** | 61.52 tok/s | 67.16 tok/s | **提速 +43.7%** |
| **抖动率 (CV Jitter %)** | **11.74%** | 45.34% | 36.56% | **平稳度提升 3.9 倍** |
| **最差轮次保底速度** | **72.88 tok/s** | 16.94 tok/s | 32.94 tok/s | **最低速度提升 4.3 倍** |
...
> **多并发核心结论**：SGLang 的 MTP-3 在 RDNA3 上 4 并发推测解码保持 **100% 稳定**——无非法显存访问、无图捕获失败、无跨流速度衰减（vLLM 原生 MTP-3 在 batch > 1 时仍会崩溃）。在当前 `llama-benchy` 0.4.0 并发模型下，总生成吞吐为 **78.5 tok/s**，峰值 **118 tok/s**；vLLM 基线列为旧工具版本实测，引用 c = 4 精确对比前请用 0.4.0 重新压测基线。
