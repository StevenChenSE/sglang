...
Prefill 调优迭代与热 L2 Autotune 修复，运行 **W4A16 草稿模型**（当前生产配置））：
数学 CoT 生成速度约 193 tok/s（GSM8K/MATH-500，3/4 正确——见下文已知的 greedy
GSM8K #1 失误），120k Agentic 会话回放平均生成速度约 119 tok/s（中位数约 113，
最差轮约 57），Prompt 处理约 1,950 tok/s（`llama-benchy` PP=2048），深度 0 生成
128 tok/s、16k 深度 109 tok/s（保持率 84.8%）。
