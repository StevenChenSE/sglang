...
- **GDN Prefill 算子调优与热态 Autotune（第 1–5 轮迭代）**：面向分块 Prefill 服务态的 GDN 算子启动配置固化——`kkt_solve`（BK=32, nw=1，较冷缓存 Autotune 选中项 **2.07×**）、`recompute_w_u`（128×128, nw=8，**1.75×**）、`fwd_h` nw=8、`fwd_o` BV=128，全部通过逐位一致性校验；并新增 RDNA 门控的热 L2 `triton.testing.do_bench` 包装器，使 Autotune 按服务实际运行的热缓存状态排序配置（Triton 默认基准会在每轮迭代前刷新 L2，按冷缓存误排）。端到端 Prompt 处理吞吐累计提升约 **+8%**（PP=2048 下约 1,810 → 1,950 tok/s）。
...
本 Fork 实测（2026-09-11，双卡 RX 7900 XTX TP=2，构建 `65b16b3df7`——含 GDN/WMMA
Prefill 调优迭代与热 L2 Autotune 修复）：数学 CoT 生成速度约 193 tok/s
（GSM8K/MATH-500，3/4 正确——见下文已知的 greedy GSM8K #1 失误），120k Agentic
会话回放平均生成速度约 119 tok/s（中位数约 113，最差轮约 57），Prompt 处理约
1,950 tok/s（`llama-benchy` PP=2048），深度 0 生成 128 tok/s、16k 深度 109 tok/s
（保持率 84.8%）。
...
| 指标 | bf16 草稿模型（第 1 节，2026-09-09） | W4A16 草稿模型（融合 KV） |
|:---|:---:|:---:|
...
**82.8 tok/s**、CV **25.3%**、最差轮次 **52.5 tok/s**——bf16 草稿模型
（2026-09-09）为 83.8 / 79.8 / 23.0% / 40.6（2026-09-11 刷新后为 118.7 / 112.8 /
23.6% / 56.9），平均速度持平或略优。
